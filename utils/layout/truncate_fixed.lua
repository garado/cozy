...
-- Fixed layout, except do not draw a widget if it does not fit *completely*.
    ...
    for index, widget in pairs(self._private.widgets) do
        ...
        if is_y then
            if index ~= widgets_nr or not self._private.fill_space then
                -- Determine actual widget height by seeing the height it'd take
                -- up if you gave it "unlimited" space
                _, actual_height = base.fit_widget(self, context, widget, w, 1000)
                h = select(2, base.fit_widget(self, context, widget, w, h))
                zero = h == 0
                if actual_height > h then skip = true end
            end
